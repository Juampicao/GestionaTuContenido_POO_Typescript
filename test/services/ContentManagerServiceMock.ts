...
export class ContentManagerServiceMock implements IContentManagerService{
    ...
    // for (let i = 0; i < this._contentItems.length; i++) {
    //     let contentItem = this._contentItems[i];

    //     if ((filter.titleOrDescription !== "" && contentItem.containDescriptionOrTitle(filter.titleOrDescription.toLocaleLowerCase()))) {
    //         _filterContentList.push(contentItem)
    //     }
        
    //     if (filter.contentType !== "" && contentItem.contentType === filter.contentType) {
    //         _filterContentList.push(contentItem)
    //     }
    
    //     // (filter.contentType !== "" && contentItem.contentType === filter.contentType) ||
    //     // (filter.tags.length !== 0 && contentItem.containTags(filter.tags))
    //     // ||
    //     // (filter.ratingSince !== IContentItemRating.Void || filter.ratingUntil !== IContentItemRating.Void && contentItem.containsRating(filter.ratingSince, filter.ratingUntil)) ||
    //     // (filter.fechaCreacionSince !== voidFilter.fechaCreacionUntil || filter.fechaCreacionUntil !== voidFilter.fechaCreacionSince && contentItem.containsFechaCreacion(filter.fechaCreacionSince, filter.fechaCreacionUntil)) ||
    //     // (filter.durationSince !== voidFilter.durationSince || filter.durationUntil !== voidFilter.durationUntil && contentItem.containsItemsBetweenTwoDurations(filter.durationSince, filter.durationUntil))
    //     {
    //         _filterContentList.push(contentItem);
    //     }
    // }
    ...
    getContentsItemsByFilter(filter: ContentItemFilter): Array<ContentItem>  {  
        ...
        for (let i = 0; i < this._contentItems.length; i++) {
            ...
            if (filter.titleOrDescription !== "") {
                ...
                if (!this._contentItems[i].containDescriptionOrTitle(filter.titleOrDescription.toLowerCase())) {     
                    passedFilter = false;
                }
            }
